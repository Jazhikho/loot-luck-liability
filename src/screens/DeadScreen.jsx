  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-white">
      ...
      <div className="max-w-md rounded-2xl border border-rose-400/20 bg-slate-900/80 p-6 text-center shadow-[0_0_60px_rgba(244,63,94,0.08)]">
        ...
        <h1 className="mb-2 text-3xl font-bold text-rose-300">Luck Finally Blinked</h1>
        ...
        <div className="mb-4 space-y-1 rounded-xl border border-slate-700 bg-slate-800/90 p-4 text-left text-sm">
          <p className="mb-2 text-base font-bold text-yellow-100">Run Summary</p>
          <p>Gold Hauled: <span className="text-amber-300">{rs.earned}g</span></p>
          <p>Monsters Slain: <span className="text-emerald-300">{rs.slain}</span></p>
          <p>Deepest Floor: <span className="text-cyan-300">{rs.deepest}</span></p>
          <p>Rooms Explored: {rs.rooms}</p>
          <p>Lost Cargo: {inv.length} items ({lostValue}g)</p>
        </div>
