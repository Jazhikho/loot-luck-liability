  ...
  return (
    <div className="rounded-xl border border-emerald-400/20 bg-slate-950/95 p-3 shadow-[0_0_30px_rgba(14,165,233,0.06)]">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-200/80">{t("ui.panels.logTitle")}</p>
          <p className="text-[11px] text-slate-500">{t("ui.panels.logSubtitle")}</p>
        </div>
        {latest && (
          <p className={`max-w-72 truncate text-[11px] ${LC[latest.type] || LC.normal}`}>
            {t("ui.common.latest")}: {latest.msg}
          </p>
        )}
      </div>
      <div
      ...
      >
        {log.length === 0 && <span className="text-slate-600">{t("ui.panels.logEmpty")}</span>}
        {log.map((entry) => (
